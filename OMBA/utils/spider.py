    ...
    def requestGet(self, url, headers=None):
        try:
            ...
            if encoding == 'ISO-8859-2':
                response = response.decode('gbk', 'ignore').encode('gb2312', 'ignore')
            elif encoding == 'GB2312':
                response = response.decode('gbk', 'ignore').encode('utf-8', 'ignore')
            elif encoding == 'EUC-TW':
                response = response.decode('gbk', 'ignore').encode('utf-8', 'ignore')
            else:
                response = response.decode(encoding, 'ignore').encode('utf-8', 'ignore')
        except Exception, e:
            print e
            return False
        return response
