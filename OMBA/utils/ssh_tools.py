...
class SSH(threading.Thread):
    def __init__(self, ssh, hostname, model=None, queue=None, thread=False, cmd=None, cmd_args=None, localPath=None, remotePath=None, remoteFile=None):
...
class SSHManage(object):
    ...
    def thread(self, model, queue, cmd=None, cmd_args=None, localPath=None, remotePath=None, remoteFile=None):
        if model == 'command':
            self.sshConn = paramiko.SSHClient()
            self.sshConn.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            self.sshConn.connect(
...
if __name__ == '__main__':
    for x in ['192.168.1.112', '192.168.1.113']:
        sshRbt = SSHManage(
            hostname=x,
            password='welliam',
            username='root',
