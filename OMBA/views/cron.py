...
from OMBA.models import Cron_Config, Server_Assets
from OMBA.utils.ssh_tools import SSHManage
from OMBA.utils import base
from OMBA.tasks.cron import recordCron
from OMBA.models import Log_Cron_Config
...
@login_required()
@permission_required('OMBA.can_add_cron_config', login_url='/noperm/')
    ...
    if request.method == "GET":
        ...
        if int(cron_status) == 1:
            try:
                ...
                #
                cron_args = """
                    name={name} minute='{minute}' hour='{hour}' day='{day}' weekday='{weekday}' month='{month}' user='{user}' job='{job}'
                """.format(
                    name=cron.cron_name,
...
@login_required()
@permission_required('OMBA.can_read_config', login_url='/noperm/')
...
@login_required()
@permission_required('OMBA.can_change_cron_config', login_url='/noperm/')
