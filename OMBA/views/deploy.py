...
def sendDeployNotice(order_id, mask):
    ...
    if order.order_status == 2:
        to_user = order.order_audit
    else:
        to_user = order.order_user
    CGroups(to_user).send(
        {
            'text': json.dumps(
                {
                    "title": "你有一条新的工单需要处理<br>",
                    "type": "info",
                    "messages": content
                }
            )
