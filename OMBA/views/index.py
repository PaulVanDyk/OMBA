...
def index(request):
    # 7天更新频率统计
    userList = Project_Order.objects.raw('''SELECT id, order_user FROM omba_project_order GROUP BY order_user;''')
    userList = [u.order_user for u in userList]
    dataList = [base.getDaysAgo(num) for num in xrange(0, 7)][::-1]    # 将日期逆序
    dateList = []
    for user in userList:
        valueList = []
        data = dict()
        for startTime in dateList:
            sql = """SELECT id, IFNULL(count(0), 0) as count FROM omba_project_order WHERE date_format(create_time, "%%y%%m%%d") = {startTime} and order_user = '{user}'""".format(startTime=startTime, user=user)
    ...
    for ms in monthList:
        ...
        data['data'] = ms
        for user in userList:
            sql = """SELECT id ,IFNULL(count(0), 0) as count FROM omba_project_order WHERE data_format(create_time, "%%Y%%m%%d") >= {startTime} and order_user = '{user}'""".format(startTime=startTime, endTime=endTime, user=user)
            userData = Project_Order.objects.raw(sql)
            if userData[0].count == 0:
...
@login_required(login_url='/login')
@permission_required('OMBA.can_change_global_config', login_url='/noperm/')
def config(request):
