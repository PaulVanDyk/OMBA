...
from OMBA.models import Assets, Email_Config, Server_Assets, NetworkCard_Assets
...
def expireAssets(**kw):
    if kw.has_key('expire') and kw.has_key('user'):
        ...
        expired = []
        expire_soon = []
        serverList = Assets.objects.raw(
            'SELECT id, management_ip, expire_date, expire_date - CURDATE() as diff from omba_assets where expire_date - CURDATE() is not null;'
        )
        for ds in serverList:
            if ds.diff < 0:
                expired.append(ds)
            elif ds.diff >= 0 and ds.diff < kw.has_key('expire'):
                expire_soon.append(ds)
        if expired or expire_soon:
            ...
            for s in expired + expire_soon:
                ...
                trs = trs + tr
            table = '''
                <table border="1" width="600" rules="all">
            ...
            try:
                ...
                return ex
            if config.subject:
                subject = "{sub} 资产过期提醒  【重要】".format(sub=config.subject)
            else:
                subject = "资产过期提醒  【重要】"
            if config.cc_user:
