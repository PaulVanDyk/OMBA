...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_wiki_tag', raise_exception=True)
...
@permission_required('OMBA.can_change_wiki_tag', raise_exception=True)
def tag_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.can_delete_wiki_tag'):
...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_wiki_category', raise_exception=True)
...
@api_view(['GET', 'PUT', 'DELETE'])
@permission_required('OMBA.can_change_wiki_category', raise_exception=True)
def category_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.can_delete_wiki_category'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
@api_view(['GET', 'PUT', 'DELETE'])
@permission_required('OMBA.can_delete_wiki_post', raise_exception=True)
def archive_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.can_delete_wiki_post'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
