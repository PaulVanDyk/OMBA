...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_cron_config', raise_exception=True)
...
@permission_required('OMBA.can_change_cron_config', raise_exception=True)
def cron_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.can_delete_cron_config'):
...
@api_view(['GET', 'PUT', 'DELETE'])
@permission_required('OMBA.delete_log_cron_config', raise_exception=True)
def cronLogsdetail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.can_delete_log_cron_config'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
