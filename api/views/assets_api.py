...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_project_assets', raise_exception=True)
    ...
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            ...
            recordAssets.delay(user=str(request.user),
                               content="添加项目名称：{project_name}".format(project_name=request.data.get("project_name")),
...
def project_detail(request, id, format=None):
    ...
    elif request.method == 'PUT' and request.user.has_perm('OMBA.can_change_project_assets'):
        serializer = serializers.ProjectSerializer(snippet, data=request.data)
        old_name = snippet.project_name
        if serializer.is_valid():
            serializer.save()
            recordAssets.delay(user=str(request.user),
                               content="修改项目为：{old_name} -> {project_name}".format(old_name=old_name,
                                                                                   project_name=request.data.get(
                                                                                       "project_name")), type="project",
                               id=id)
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    elif request.method == 'DELETE' and request.user.has_perm('OMBA.can_delete_rroject_assets'):
        if not request.user.has_perm('OMBA.can_delete_rroject_Assets'):
...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_service_assets', raise_exception=True)
...
@api_view(['GET', 'PUT', 'DELETE'])
@permission_required('OMBA.can_change_service_assets', raise_exception=True)
def service_detail(request, id, format=None):
    ...
    elif request.method == 'DELETE' and request.user.has_perm('OMBA.can_delete_assets'):
        if not request.user.has_perm('OMBA.can_delete_service_assets'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
        recordAssets.delay(user=str(request.user),
...
@api_view(['GET', 'DELETE'])
@permission_required('OMBA.read_log_assets', raise_exception=True)
def assetsLog_detail(request, id, format=None):
    ...
    elif request.method == 'DELETE' and request.user.has_perm('OMBA.delete_log_assets'):
        if not request.user.has_perm('OMBA.delete_log_assets'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
@api_view(['GET', 'POST'])
@permission_required('OMBA.add_group', raise_exception=True)
def group_list(request, format=None):
    """
    List all order, or create a server assets order.
    """
    if not request.user.has_perm('OMBA.read_group'):
        return Response(status=status.HTTP_403_FORBIDDEN)
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.change_group'):
            return Response(status=status.HTTP_403_FORBIDDEN)
...
@api_view(['GET', 'PUT', 'DELETE'])
@permission_required('OMBA.change_group', raise_exception=True)
def group_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.delete_group'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_zone_assets', raise_exception=True)
...
@api_view(['GET', 'PUT', 'DELETE'])
@permission_required('OMBA.can_change_zone_assets', raise_exception=True)
def zone_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.can_delete_zone_assets'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_line_assets', raise_exception=True)
...
@api_view(['GET', 'PUT', 'DELETE'])
@permission_required('OMBA.can_change_line_assets', raise_exception=True)
def line_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.can_delete_line_assets'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_raid_assets', raise_exception=True)
...
@api_view(['GET', 'PUT', 'DELETE'])
@permission_required('OMBA.can_change_raid_assets', raise_exception=True)
def raid_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.can_delete_raid_assets'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_assets', raise_exception=True)
...
@api_view(['GET', 'PUT', 'DELETE'])
@permission_required('OMBA.can_change_assets', raise_exception=True)
def asset_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.delete_asset_assets'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_server_assets', raise_exception=True)
...
@api_view(['GET', 'PUT', 'DELETE'])
@permission_required('OMBA.can_change_server_assets', raise_exception=True)
def asset_server_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.can_delete_server_assets'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
        try:
            assets_snippet = Assets.objects.get(id=snippet.assets.id)
...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_net_assets', raise_exception=True)
...
@api_view(['GET', 'PUT', 'DELETE'])
@permission_required('OMBA.can_change_net_assets', raise_exception=True)
def asset_net_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.delete_net_assets'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
        try:
            assets_snippet = Assets.objects.get(id=snippet.assets.id)
