...
from OMBA.tasks.deploy import recordProject
...
@api_view(['GET', 'POST'])
@permission_required('OMBA.can_add_project_config', raise_exception=True)
...
@permission_required('OMBA.can_delete_project_config', raise_exception=True)
def deploy_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.delete_project_config'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        recordProject.delay(
            project_user=str(request.user),
            project_id=id,
            project_name=snippet.project.project_name,
...
@api_view(['GET', 'DELETE'])
@permission_required('OMBA.delete_log_project_config', raise_exception=True)
def deployLogs_detail(request, id, format=None):
    ...
    if request.method == 'GET':
        ...
        if not request.user.has_perm('OMBA.delete_log_project_config'):
            return Response(status=status.HTTP_403_FORBIDDEN)
        snippet.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class OrderList(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if str(user) == str(username):
            return Project_Order.objects.filter(
                Q(order_user=user) | Q(order_audit=user),
                order_status__in=[0, 2]
            ).order_by("id")
        else:
            return []
