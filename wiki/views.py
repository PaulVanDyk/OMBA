...
@login_required()
@permission_required('OMBA.can_add_wiki_post', login_url='/noperm/')
...
@login_required()
@permission_required('OMBA.can_edit_wiki_post', login_url='/noperm/')
...
@login_required()
@permission_required('OMBA.can_read_wiki_post', login_url='/noperm/')
def article_index(request):
    ...
    dateList = Post.objects.raw(
        'SELECT id,DATE_FORMAT( created_time, "%%Y/%%m" ) as cdate , COUNT(*) as count FROM omba_wiki_post GROUP BY DATE_FORMAT( created_time, "%%Y/%%m" );'
    )
    return render(
        request,
        'wiki/wiki_base.html',
...
def article_archive(request, month):
    try:
        ...
        return render(
            ...
            {
                "user": request.user,
                "errorInfo": "归档时间不存在: {ex}".format(ex=ex)
            }
        )
    categoryList = []
    for ds in Category.objects.all():
        ds.post_count = Post.objects.filter(category=ds).count()
        categoryList.append(ds)
    postList = Post.objects.raw(
        '''SELECT * FROM omba_wiki_post WHERE created_time>='{startime}' AND created_time<='{endtime}' ORDER BY id DESC;'''.format(
            startime=str(archiveDate[0]) + ' 00:00:00',
            endtime=str(archiveDate[1]) + ' 23:59:59'
        )
    )
    dateList = Post.objects.raw(
        'SELECT id,DATE_FORMAT( created_time, "%%Y/%%m" ) as cdate , COUNT(*) as count FROM omba_wiki_post GROUP BY DATE_FORMAT( created_time, "%%Y/%%m" );'
